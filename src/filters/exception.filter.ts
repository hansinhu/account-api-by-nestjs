...
export class CustomExceptionFilter implements ExceptionFilter {
    ...
    if (exception.name === 'EntityNotFound' || exception.status === 404 || exception.status === 403) {
      ...
      response.status(409).json({
        error: {
          code: 'AlreadyExists',
          message: exception.code === 'ER_DUP_ENTRY' ? `Duplicate term: ${exception['parameters'][1]}` : 'This resource already exists',
        },
      })
      ...
      response.status(400).json({
        error: {
          code: 'BadRequest',
          message: exception.message.message || 'Your request seems to be invalid or malformed',
        },
      })
