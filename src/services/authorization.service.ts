...
export default class AuthorizationService {
  ...
  async getPayloadClient(clientWithSecret) {
    const { clientId, clientSecret } = clientWithSecret

    const client = await this.projectClientRepo.findOneOrFail({ where: { id: clientId } })
    const valid = await new Promise((resolve, reject) => {
      bcrypt.compare(clientSecret, client.encryptedSecret.toString('utf8'), (err, same) => {
        if (err) {
          reject(err)
        } else {
          resolve(same)
    ...
    let project_client = new ProjectClient()
    project_client.id = client.id
